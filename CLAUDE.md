...
5. 各エージェントの完了後、即座に `SendMessage(type: "shutdown_request")` でシャットダウンする
6. チームリーダーは **TaskList** で進捗を確認し、blockedBy が解消されたタスクから次のエージェントを起動する
...
- 各エージェントはタスク完了後、即座にシャットダウンする（`SendMessage(type: "shutdown_request")` を送信）
- フィードバックループで再実行が必要な場合は、新しいエージェントを起動する（既存エージェントの再利用はしない）
- 再実行時はレビューの指摘内容を `prompt` に含めることで、コンテキストを引き継ぐ

### チームの終了

全ワークフロー完了後:
1. 全エージェントがシャットダウン済みであることを確認する（各ステップで即シャットダウンしているため、通常は残存しない）
2. **TeamDelete** でチームとタスクリストを削除
